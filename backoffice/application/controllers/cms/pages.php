...
class Pages extends CI_Controller{
	...
	*/
	...
	public function view(){
		...
		$totCount = $this->cms_model->getTotStaticPagesCount();
		$start = $this->uri->segment(4,0);
		$this->load->library('pagination');
		$config['base_url'] 	= base_url()."cms/pages/view/";
		$config['total_rows'] 	= $totCount;		
		$config['per_page']     = $this->config->item('limit'); 
		$config['cur_page']     = $start;
		$this->pagination->initialize($config);	
		$data['results']	    = $this->cms_model->getAllStaticPages($config["per_page"],$start);
		$data['pagination']     = $this->pagination->create_links('view');
		$this->load->view('cms/pages/view',$data);   
	}
