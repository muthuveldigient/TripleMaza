	...
	$("#delete").click(function(){
		...
		if(chkBoxArray!=''){
			if (confirm('Are you sure. You want to delete selected items ?')){
				$.ajax({
					url: "<? echo base_url(); ?>games/history/game/deactivateTournament?chkarr="+chkBoxArray,
					type: "post",
					async: false,
					//data: $('.wBatch:checked').serialize(),
					success: function(data) {
						window.location='<? echo base_url(); ?>games/history/game/editgame?rid=71&msg=111'; 
					}
				});
			}
		}else{
...
function changestatus(id){
    ...
    var url='<?php echo base_url()."games/history/game/activateTournament/"?>'+id;
...
if(isset($results)){ 
	if(count($results)>0 && is_array($results)){
		...
		//call model to get Tournament Name,currency type.
			$resvalue['GAME_NAME']	= '<a href="'.base_url().'games/history/game/edit/'.$results[$i]->TOURNAMENT_ID.'?rid='.$rid.'">'.$results[$i]->GAME_NAME.'</a>';
