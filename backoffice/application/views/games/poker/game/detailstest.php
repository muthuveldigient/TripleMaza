        ...
        <div class="table_card">
    	   ...
    	   <div class="rake">
            <div style="width:250px;float:left;"> 
             ...
             <div style="width:250px;float:left;"><h3>Rake(Headsup):</h3><span><?php echo floor($rake->RAKE1);?> %</span></div>
			...
			if(count($results)>0 && is_array($results)){
				for($i=0;$i<count($results);$i++){
					if($results[$i]->PLAY_DATA){
								...
								if($results[0]->MINIGAMES_TYPE_NAME=='omeha'){
                        	...
                        	<?PHP 
							  ...
							  if(count($playdata['playResult']['userAction'])>0){
										if(count($playdata['playResult']['userAction']['ROUND:1'])>0){
											for($mn=0;$mn<count($playdata['playResult']['userAction']['ROUND:0']);$mn++){
												$round='ROUND:1';
												$turn='TURN:'.$mn;
