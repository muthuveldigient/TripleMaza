...
<div class="content_wrap">
   ...
   <form action="<?php echo base_url()?>games/history/game/history?rid=<?php echo $rid?>" method="post" name="tsearchform" id="tsearchform" onsubmit="return chkdatevalue();">
    <table width="100%" cellpadding="0" cellspacing="0" bgcolor="#993366" class="searchWrap">
    <tr>
      <td><table width="100%" class="ContentHdr">
          <tr>
            <td><strong>Casino Game History</strong></td>
          </tr>
        </table>
  ...
  <td><form action="<?php echo base_url();?>games/history/game/history?rid=<?php echo $rid;?>" method="post" name="clrform" id="clrform">
	...
	if(isset($results)){ 
		if(count($results)>0 && is_array($results)){
			for($i=0;$i<count($results);$i++){
				...
				if($userName){
					$resvalue['USERNAME']	= '<a href="'.base_url().'games/history/game/userhistory/'.$userName.'?gameid='.$_REQUEST[gameID].'&sdate='.$_REQUEST[START_DATE_TIME].'&edate='.$_REQUEST[END_DATE_TIME].'&intRefNo='.$_REQUEST[intRefNo].'&rid=72">'.$userName.'</a>';
				}else{
					$resvalue['USERNAME']	= '-';
				}
				if($tot_Bets)
					$resvalue['TOT_BETS']	= number_format($tot_Bets,2, '.', '');
				else
					$resvalue['TOT_BETS']	= '-';
				if($tot_Wins)
					$resvalue['TOT_WINS']	= number_format($tot_Wins,2, '.', '');
				else
					$resvalue['TOT_WINS']	= '-';	
				if($tot_Refunds)
					$resvalue['TOT_REFUNDS']	= number_format($tot_Refunds,2, '.', '');
				else
					$resvalue['TOT_REFUNDS']	= '-';		
				if($end_Points)
					$resvalue['TOT_END_POINTS']	= number_format($end_Points,2, '.', '');
				else
...
$sumendpts = $sumbets - $sumwin - $sumrefund;
...
<span style="position:relative;top:8px;left:10px"> <b> Total Record: <font color="#FF3300">(<?php echo $total_rows; ?>) </font></b> &nbsp;&nbsp;&nbsp; </span>
...
<span style="position:relative;top:8px;left:10px"> <b> Total End Points: <font color="#FF3300">(<?php echo number_format($sumendpts, 2, '.', ''); ?>) </font></b> &nbsp;&nbsp;&nbsp; </span>
