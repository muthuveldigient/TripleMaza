...
<div class="data-list">
	...
	jQuery("#grid3").jqGrid({
		...
		subGridRowExpanded: function(subgrid_id, row_id) {
			...
			var rowData = $("#grid3").getRowData(row_id);
			var partner_name = rowData.PARTNER;
			subgrid_table_id = subgrid_id+"_t";
			pager_id = "p_"+subgrid_table_id;
			var aurls = '<?php echo base_url()."reports/ajaxgrid/gameinfo?q=2&GAMES_TYPE=$GAMES_TYPE&PARTNER_TYPE=$PARTNER_TYPE3&START_DATE_TIME=$START_DATE_TIME&END_DATE_TIME=$END_DATE_TIME&keyword=Search"?>&AGENT_LIST='+row_id; 
		...
		loadComplete: function () {
			var mydata = <?php echo json_encode($arrs2);?>;
			var rowid =0;
			for(var i=0;i< mydata.length;i++) {
				rowid = mydata[i].id;
				jQuery("#grid3").jqGrid('addRowData',rowid,mydata[i]);
			}
			var playpointsSum = jQuery("#grid3").jqGrid('getCol', 'PLAY_POINTS', false, 'sum').toFixed(2);
			var winpointsSum = jQuery("#grid3").jqGrid('getCol', 'WIN_POINTS', false, 'sum').toFixed(2);
			var margin = jQuery("#grid3").jqGrid('getCol', 'MARGIN', false, 'sum').toFixed(2);
			var net = jQuery("#grid3").jqGrid('getCol', 'NET', false, 'sum').toFixed(2);
			jQuery("#grid3").jqGrid('footerData','set', {PARTNER: 'Total:', PLAY_POINTS: playpointsSum,WIN_POINTS:winpointsSum, MARGIN:margin, NET:net    });	
		}
	});

		

	jQuery("#grid3").jqGrid('navGrid','#pagersg11',{add:false,edit:false,del:false});
