...
<div class="content_wrap">
   ...
   <form action="<?php echo base_url()?>games/game/history?rid=<?php echo $rid?>" method="post" name="tsearchform" id="tsearchform" onsubmit="return chkdatevalue();">
  ...
  <td><form action="<?php echo base_url();?>games/game/history?rid=<?php echo $rid;?>" method="post" name="clrform" id="clrform">
	...
	if(isset($results)){ 
		if(count($results)>0 && is_array($results)){
			for($i=0;$i<count($results);$i++){
				...
				if($userName){
					$resvalue['USERNAME']	= '<a href="'.base_url().'games/game/userhistory/'.$userName.'?gameid='.$_REQUEST[gameID].'&sdate='.$_REQUEST[START_DATE_TIME].'&edate='.$_REQUEST[END_DATE_TIME].'&intRefNo='.$_REQUEST[intRefNo].'&rid=72">'.$userName.'</a>';
				}else{
					$resvalue['USERNAME']	= '-';
				}
				if($tot_Bets)
