...
class Game extends CI_Controller{
	...
	*/
	...
	public function view($gameTypeID=''){	
		...
		$config = array();
		$config["base_url"] 	= base_url()."games/game/view";
		...
		$data = array();
				...
				$data['PLAYER_ID'] 			= $name;
				$config["base_url"] 		= base_url()."games/game/view/".$name."";
				$config["per_page"] 		= $this->config->item('limit');
	...
	*/
	...
	public function listgame(){
		...
		$config = array();
		$config["base_url"] 	= base_url()."games/game/listgame";
		$config["per_page"] 	= $this->config->item('limit');
		...
		if($this->input->get_post('keyword',TRUE)=="Search" or $this->input->get('keyword',TRUE)=="Search"){
			...
			if(count($data)){
				...
				redirect('games/game/editgame?rid=42');
		}else{
				redirect('games/game?rid=41');
		}
	}
	...
	*/
	...
	public function history(){	
		...
		$config = array();
		$config["base_url"] 	= base_url()."games/game/history";
		$config["per_page"] 	= $this->config->item('limit');
	...
	public function userhistory(){	
		...
		$config = array();
		$config["base_url"] 	= base_url()."games/game/userhistory";
		$config["per_page"] 	= $this->config->item('limit');
		$config['suffix'] = '?chk=48&playerID='.$searchdata['playerID'].'&gameID='.$searchdata['gameID'].'&intRefNo='.$searchdata['intRefNo'].'&START_DATE_TIME='.$searchdata['START_DATE_TIME'].'&END_DATE_TIME='.$searchdata['END_DATE_TIME'].'&keyword=Search&rid='.$this->input->get('rid');
		$config['first_url']	= $config['base_url'].$config['suffix'];
		$config["uri_segment"] 	= 2;
		...
		if($this->input->get_post('keyword',TRUE)=="Search" or $this->input->get('keyword',TRUE)=="Search"){
			...
			$data = array();
				...
				$data['activeGames'] 	= $this->game_model->getListOfActiveGames();
				$config["base_url"] 	= base_url()."games/game/userhistory/".$name."";
				$config["per_page"] 	= $this->config->item('limit');
