...
<div class="content_wrap">
   ...
   <form action="<?php echo base_url()?>games/history/game/userhistory?rid=<?php echo $rid?>" method="post" name="tsearchform" id="tsearchform" onsubmit="return chkdatevalue();">
    <table width="100%" cellpadding="0" cellspacing="0" bgcolor="#993366" class="searchWrap">
    <tr>
      <td><table width="100%" class="ContentHdr">
          <tr>
            <td><strong>User Casino Game History</strong></td>
          </tr>
        </table>
        <table width="100%" cellpadding="10" cellspacing="10">
        <tr>
          <td width="40%"><span class="TextFieldHdr">User ID:</span><br />
            <label>
         <input type="text" name="playerID" id="playerID" class="TextField" value="<?PHP if(isset($_REQUEST['playerID'])){ echo trim($_REQUEST['playerID']); }elseif($this->uri->segment(5)){ echo trim($this->uri->segment(5)); } ?>" tabindex="4" >
            </label>
          </td>
          ...
          <td width="30%"><span class="TextFieldHdr">Game Name:</span><br />
            	<label>
            		<select name="gameID" id="gameID" class="ListMenu">
                    	...
                    	<option  value="<?php echo $gameNames->MINIGAMES_NAME; ?>" <?php if(isset($_REQUEST['gameID']) && $_REQUEST['gameID']== "$gameNames->MINIGAMES_NAME"){ echo "selected"; }elseif(isset($_REQUEST['gameid']) && $_REQUEST['gameid']== "$gameNames->MINIGAMES_NAME"){ echo "selected";} ?>><?php echo ucfirst(strtolower($gameNames->DESCRIPTION)); ?></option>
                    	<?php } ?>
                  	</select>
                </label>            
          </td>
          <td width="40%"><span class="TextFieldHdr">Hand Id:</span><br />
            <label>
            <input type="text" name="intRefNo" id="intRefNo" class="TextField" value="<?PHP if(isset($_REQUEST['intRefNo'])) echo trim($_REQUEST['intRefNo']); ?>" tabindex="1" >
            </label></td>
          <td width="30%"></td>
        </tr>
        <tr>
                <td width="40%"><span class="TextFieldHdr">From:</span><br />
                  ...
                  <?php
					...
					elseif($_REQUEST['sdate'])
						$START_DATE_TIME = $_REQUEST['sdate'];
					else
                ...
                <td width="40%"><span class="TextFieldHdr">To:</span><br />
                  <label>
                   <?php
					...
					elseif($_REQUEST['edate'])
						$END_DATE_TIME =$_REQUEST['edate'];
					else
  ...
  <td><form action="<?php echo base_url();?>games/history/game/userhistory?rid=<?php echo $rid;?>" method="post" name="clrform" id="clrform">
		...
		if(count($results)>0 && is_array($results)){
			for($i=0;$i<count($results);$i++){
				if($results[$i]->USERNAME)
					...
					$resvalue['USERNAME']	= '-';
				$gameName = $this->game_model->getGameNameByMinigamesId($results[$i]->INTERNAL_REFERENCE_NO);
				if($results[$i]->INTERNAL_REFERENCE_NO){
					$resvalue['INTERNAL_REFERENCE_NO']	= '<a href="#" onclick="NewWindow(\''.base_url().'games/history/gamedetails/view/'.$results[$i]->INTERNAL_REFERENCE_NO.'?rid=73\',\'Game Details\',\'1024\',\'900\',\'yes\')">'.$results[$i]->INTERNAL_REFERENCE_NO.'</a>';
				}else{
...
<style>
    ...
    <?php if($_REQUEST['intRefNo']==''){ ?>
    <b> Total Records: <font color="green">(<?php if($totRecords) echo $totRecords; ?>)</font></b> &nbsp;&nbsp;&nbsp;
    <b> Total Bet: <font color="#FF3300">(<?php if($totals[0]->BET_POINTS) echo number_format($totals[0]->BET_POINTS,2,".",""); ?>)</font></b> &nbsp;&nbsp;&nbsp;
    <b> Total Win: <font color="#FF3300">(<?php if($totals[0]->WIN_POINTS) echo number_format($totals[0]->WIN_POINTS,2,".","");?>)</font></b>&nbsp;&nbsp;&nbsp;
    <b> Total Refund: <font color="#FF3300">(<?php if($totals[0]->REFUND_POINTS) echo number_format($totals[0]->REFUND_POINTS,2,".","");?>)</font></b>
    <?php } ?>
    ...
    <div class="tableListWrap">
      <div class="data-list">
        ...
        <script type="text/javascript">
                jQuery("#list4").jqGrid({
                    datatype: "local",
					colNames:['User Name','Internal Reference No','Current Tot Bal','Bet Points','Win Points','Refund Points','Closing Tot Bal','Transaction date'],
                    colModel:[
