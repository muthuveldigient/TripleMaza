	...
	if(count($results)>0 && is_array($results)){
		for($i=0;$i<count($results);$i++){
			...
			if($results[$i]->PLAY_GROUP_ID)
				$resvalue['PLAY_GROUP_ID']		= '<a href="#" onclick="NewWindow(\''.base_url().'games/history/gamedetails/details/'.$results[$i]->PLAY_GROUP_ID.'/'.$results[$i]->GAME_ID.'?rid=45\',\'Game Details\',\'1000\',\'600\',\'yes\')">'.$results[$i]->PLAY_GROUP_ID.'</a>';
			else
