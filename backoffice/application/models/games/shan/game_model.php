...
class Game_model extends CI_Model
{
	...
	public function getmobAndharBaharPlayDetails($handId,$gameName){
		$this->load->database();
		$query = $this->db3->query("select andarbahar_play.*,user.USERNAME from andarbahar_play, user where andarbahar_play.USER_ID = user.USER_ID AND GAME_ID='".$gameName."' and INTERNAL_REFFERENCE_NO='".$handId."'");
		$gamePlay = $query->result();
...
$querycnt=$this->db2->query("select sum(p.RAKE) as RAKE from shan_play_backup p,user u where u.user_id=p.user_id and u.partner_id in ($partnerids) and p.ended is not null ");			
	...
	public function insertGame($data=array()){
	   ...
	   /** tracking info */
		...
		$arrTraking["ACTION_NAME"]  ="Create Game";
			...
			$insertData = array(
			   ...
			   'MODIFIED_BY'			=> $_SERVER['REMOTE_ADDR']."-".$this->session->userdata['partnerid']
				...
				if($res){
					...
					$arrTraking["CUSTOM1"]      =json_encode(array('formData'=>$_REQUEST,'message'=>'Failure'));
					$this->db->insert("tracking",$arrTraking);	
					$err = 5;
				}
		}else{
 ...
 public function getDynamicTableID($intRefNo) {
  ...
  return $gameDetailsInfo;  
	...
	public function deleteGame($gameId){
		...
		$arrTraking["DATE_TIME"] = date('Y-m-d h:i:s');
		$arrTraking["USERNAME"]     =$this->session->userdata('partnerusername');
		$arrTraking["ACTION_NAME"]  ="Delete Game";
		$arrTraking["SYSTEM_IP"]    =$_SERVER['REMOTE_ADDR'];				
		$arrTraking["REFERRENCE_NO"]=uniqid();
		$arrTraking["STATUS"]       =1;
		$arrTraking["LOGIN_STATUS"] =1;
		...
		if($res){
			...
			$arrTraking["CUSTOM1"]      =json_encode(array('formData'=>$_REQUEST,'message'=>$returnMsg));
			$this->db->insert("tracking",$arrTraking);	
		}else{
			$arrTraking["CUSTOM1"]      =json_encode(array('formData'=>$_REQUEST,'message'=>$returnMsg));
			$this->db->insert("tracking",$arrTraking);	
	...
	public function getListOfActiveGames(){
		$this->load->database();
		$res = $this->db2->query("select MINIGAMES_NAME,DESCRIPTION from minigames where STATUS=1 AND minigames_id not in (62) order by DESCRIPTION");
	...
	public function getGamesHistoryBySearchCriteria($config,$data){
		...
		$this->db3->select('u.USERNAME,sum(vth.BET_POINTS) as TOT_BETS,sum(vth.WIN_POINTS) as TOT_WINS,sum(vth.REFUND_POINTS) as TOT_REFUNDS')
		->from('view_transaction_history vth');
		$this->db3->join('user u ', 'vth.USER_ID = u.USER_ID');
		if($parent_type > 0){
			$partnerids  = $this->Agent_model->getAllChildIds_kiosk(); 
			$partnerids = explode(",",$partnerids);
...
$cards = '{"userHands":[["'.$p11.'","'.$p12.'"],["'.$p21.'","'.$p22.'"],["'.$p31.'","'.$p32.'"],["'.$p41.'","'.$p42.'"],["'.$p51.'","'.$p52.'"],["'.$p61.'","'.$p62.'"],["'.$p71.'","'.$p72.'"],["'.$p81.'","'.$p82.'"]],"userHands1":[["'.$p13.'"],["'.$p23.'"],["'.$p33.'"],["'.$p43.'"],["'.$p53.'"],["'.$p63.'"],["'.$p73.'"],["'.$p83.'"]]}';
	...
	public function scratchcard_play($handId){
		$this->load->database();
		$query = $this->db3->query("select scratchcard_play.*,user.USERNAME from scratchcard_play, user where scratchcard_play.USER_ID = user.USER_ID AND GAME_ID='tinkuscratchcard5' and INTERNAL_REFERENCE_NO='".$handId."'");	
		$gamePlay = $query->result();
		
		foreach($gamePlay as $key=>$val){
			$querysub = $this->db2->query("select PRICE from scratch_price p,minigames m where m.MINIGAMES_ID=p.MINIGAMES_ID and m.MINIGAMES_NAME='".$handId."'");	
			$gamePlaysub = $querysub->result_array();
			$gamePlay[$key]->PRICE = ""; 
			$gamePlay[$key]->usertickets = ""; 
			if(!empty($gamePlay)){
				$usertickets=count($tot['scratchCardResult']['scratchCardTicketList']);
				$gamePlay[$key]->PRICE = $gamePlaysub[0]['PRICE']; 
				$tot=json_decode($val->PLAY_DATA, true);   
				$userticketss=$tot['scratchCardResult']['scratchCardTicketList'];
				$gamePlay[$key]->userticketss = $userticketss;
				$gamePlay[$key]->brandname = $tot['scratchCardResult']['brandName'];
				$gamePlay[$key]->split_details = $tot['scratchCardResult']['ticketBatchSplitId'];
				$gamePlay[$key]->userticketiddetails = $tot['scratchCardResult'];
			}
	...
	public function slotreel5_nordic_t1_play($handId,$get){
		$this->load->database();
		if($get == 'slotreel5_nordic_t1'){
		$query = $this->db3->query("select slotreel5_nordic_t1_play.*,user.USERNAME from slotreel5_nordic_t1_play, user where slotreel5_nordic_t1_play.USER_ID = user.USER_ID AND GAME_ID='slotreel5_nordic_t1' and INTERNAL_REFERENCE_NO='".$handId."'");
		}
		if($get == 'slotreel5_pharaoh_t2'){
		$query = $this->db3->query("select slotreel5_pharaoh_t2_play.*,user.USERNAME from slotreel5_pharaoh_t2_play, user where slotreel5_pharaoh_t2_play.USER_ID = user.USER_ID AND GAME_ID='slotreel5_pharaoh_t2' and INTERNAL_REFERENCE_NO='".$handId."'");	
		}
		//echo $this->db3->last_query();
		$gamePlay = $query->result();// print_r($gamePlay);
		return $gamePlay;		
	}	
	public function andarbahar_play($handId,$get){
		
		$this->load->database();
		$query = $this->db3->query("select * from andarbahar_play where INTERNAL_REFFERENCE_NO='".$handId."'");
		$gamePlay = $query->result();
		foreach($gamePlay as $key=>$val){
			$USER_ID = $val->USER_ID;
			$querysub = $this->db3->query("SELECT USER_ID,USERNAME FROM user WHERE USER_ID=".$USER_ID."");	
			$gamePlaysub = $querysub->result();
			$gamePlay[$key]->USERNAME = $gamePlaysub[0]->USERNAME;
		}
		//print_r($gamePlay);
		return $gamePlay;	
	}
	public function horserace($handId,$get){
		$this->load->database();
		if($get=="horserace"){
			$query = $this->db3->query("select horserace_play.*,user.USERNAME from horserace_play, user where horserace_play.USER_ID = user.USER_ID AND GAME_ID='horserace' and INTERNAL_REFERENCE_NO='".$handId."'");
		}else if($get=="horseracetimer"){
			$query = $this->db3->query("select horserace_play.*,user.USERNAME from horserace_play, user where horserace_play.USER_ID = user.USER_ID AND GAME_ID='horseracetimer' and INTERNAL_REFERENCE_NO='".$handId."'");
		}else{
			$query = $this->db3->query("select horserace_play.*,user.USERNAME from horserace_play, user where horserace_play.USER_ID = user.USER_ID AND GAME_ID='mobhorseracetimer' and INTERNAL_REFERENCE_NO='".$handId."'");
		}
	...
	public function getmobTripleChanceTimerPlayDetails($handId,$get){
		$this->load->database();
		if($get=="triplechancetimer"){
		...
		}else if($get=="mobtriplechancetimer"){
			...
			$query = $this->db3->query("select triplechance_play.*,user.USERNAME from triplechance_play, user where triplechance_play.USER_ID = user.USER_ID AND GAME_ID='wintriplechancetimer' and INTERNAL_REFERENCE_NO='".$handId."'");
		}
		//echo $this->db3->last_query(); 
		$gamePlay = $query->result();
		return $gamePlay;		
	}
	public function getmobvip36PlayDetails($handId,$gamename){
		$this->load->database();
		$query = $this->db3->query("select roulette_play.*,user.USERNAME from roulette_play, user where roulette_play.USER_ID = user.USER_ID AND GAME_ID='".$gamename."' and INTERNAL_REFFERENCE_NO='".$handId."'");
		$gamePlay = $query->result();
		return $gamePlay;		
	}
	public function getTripleChanceTimerPlayDetails($handId, $gamename){
		
		$this->load->database();
		$this->db3->select("mp.*,u.USERNAME")->from('triplechance_play as mp');
		$this->db3->join('user as u','mp.USER_ID = u.USER_ID');
		if(!empty($gamename)){
			$this->db3->where('mp.GAME_ID', $gamename);
		}
		if(!empty($handId)) {
			$this->db3->where('mp.INTERNAL_REFERENCE_NO', $handId);	
		}
		
		$browseSQL = $this->db3->get();
		$gamePlay  = $browseSQL->result();
...
FROM shan_wallet_transaction s,transaction_type t,shan_tournament_tables st  where st.TOURNAMENT_ID=s.ROOM_ID AND s.TRANSACTION_TYPE_ID=t.TRANSACTION_TYPE_ID and user_id=14462");
  ...
  die;
	...
	public function getmobInsideOutsideTimerPlayDetails($handId){
		$this->load->database();
		$query = $this->db3->query("select andarbahar_play.*,user.USERNAME from andarbahar_play, user where andarbahar_play.USER_ID = user.USER_ID AND GAME_ID='mobinsideoutsidetimer' and INTERNAL_REFFERENCE_NO='".$handId."'");
		//echo $this->db3->last_query();exit;
		$gamePlay = $query->result();
		return $gamePlay;		
	}
	
	public function getMegaWheelPlayDetails($gamename, $handId){
		$this->load->database();
		$this->db3->select("mp.*,u.USERNAME")->from('megawheel_play as mp');
		$this->db3->join('user as u','mp.USER_ID = u.USER_ID');
		if(!empty($gamename)){
			$this->db3->where('mp.GAME_ID', $gamename);
		}
	...
	public function getSpin2WinProPlayDetails($gamename, $handId){
		$this->load->database();
		$this->db3->select("mp.*,u.USERNAME")->from('spin2win_pro_play as mp');
		$this->db3->join('user as u','mp.USER_ID = u.USER_ID');
		if(!empty($gamename)){
			$this->db3->where('mp.GAME_ID', $gamename);
		}
	...
	public function getMobCricketSlotPlayDetails($handId){
		$this->load->database();
		$query = $this->db3->query("select mobslotreel5_nordic_t1_play.*,user.USERNAME from mobslotreel5_nordic_t1_play, user where mobslotreel5_nordic_t1_play.USER_ID = user.USER_ID AND GAME_ID='mobslotreel5_nordic_t1' and mobslotreel5_nordic_t1_play.INTERNAL_REFERENCE_NO='".$handId."'");	
		$gamePlay = $query->result();
		return $gamePlay;	
	}
	public function getMobRaiseofDevilsPlayDetails($handId){
		$this->load->database();
		$query = $this->db3->query("select mobslotreel5_nordic_t2_play.*,user.USERNAME from mobslotreel5_nordic_t2_play, user where mobslotreel5_nordic_t2_play.USER_ID = user.USER_ID AND GAME_ID='mobslotreel5_nordic_t2' and mobslotreel5_nordic_t2_play.INTERNAL_REFERENCE_NO='".$handId."'");	
		$gamePlay = $query->result();
		return $gamePlay;	
	}
	
	public function getHorseTimerDetails($handId, $gameId){
		$gamePlay = array();
		$this->load->database();
		if(!empty($handId) && !empty($gameId)){
			$query = $this->db3->query("select horserace_play.*,user.USERNAME from horserace_play, user where horserace_play.USER_ID = user.USER_ID AND GAME_ID='".$gameId."'and INTERNAL_REFERENCE_NO='".$handId."'");
			$gamePlay = $query->result();
			//echo $this->db3->last_query();exit;
		}
		return $gamePlay;		
	}
	public function getTripleDicePlayDetails($handId, $gameId){
		$gamePlay = array();
		$this->load->database();
		if(!empty($handId) && !empty($gameId)){
			$query = $this->db3->query("select tripledice_play.*,user.USERNAME from tripledice_play, user where tripledice_play.USER_ID = user.USER_ID AND GAME_ID='".$gameId."'and INTERNAL_REFERENCE_NO='".$handId."'");
			$gamePlay = $query->result();
			//echo $this->db3->last_query();exit;
		}
		return $gamePlay;		
	...
	/* public function getMegaWheelPlayDetails($handId, $gameId){
		$gamePlay = array();
		$this->load->database();
		if(!empty($handId) && !empty($gameId)){
			$query = $this->db3->query("select megawheel_play.*,user.USERNAME from megawheel_play, user where megawheel_play.USER_ID = user.USER_ID AND GAME_ID='".$gameId."'and INTERNAL_REFERENCE_NO='".$handId."'");
			$gamePlay = $query->result();
			//echo $this->db3->last_query();exit;
		}
		return $gamePlay;		
	...
	public function getGameDetails($handId){
		$gameCode = substr($handId, 0, 3); 
		$this->load->database();
		$query = $this->db2->query('select * from minigames where REF_GAME_CODE="'.$gameCode.'"');
		$gameNAme = $query->result();
		return $gameNAme;
    }
	...
	public function getWinBigBossSumDetails($handId, $gameId){
		$this->load->database();
		...
		$browseSQL = $this->db2->get("lucky7_lotto_tickets as t");
		$gameInfo  =  $browseSQL->row_object();
		//echo $this->db2->last_query();exit;
		return $gameInfo;		
	}
	public function getWinBigBossPlayDetails($handId, $gameId){
		$this->load->database();
	...
	public function getWinBigBossGameDetails(){
		...
		$gameInfo  =  $browseSQL->result();
		//echo $this->db2->last_query();exit;
		//echo '<pre>';print_r($gameInfo);exit;
		return $gameInfo;	
	} 
