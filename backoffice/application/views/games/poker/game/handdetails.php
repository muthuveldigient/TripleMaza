...
.searchgame_cardcnt{	overflow:hidden;	background-color:#d2f0ff;	padding:9px;	min-height:90px;	text-align:center;	min-width:76px;	}
...
.rake span{	float:left;	line-height:20px;	margin-left:5px;	font-size:13px;		}
        ...
        <div class="table_card">
            <div class="tablecard_cnt">
             ...
             <?php 
			 $handID =  $this->uri->segment(5);
			 $realrevenue  =  $this->uri->segment(6);
			 $bonusrevenue =  $this->uri->segment(7);
			$real_revenue = base64_decode($realrevenue);
			$bonus_revenue = base64_decode($bonusrevenue);
			$CI = &get_instance();
			$this->db3 = $CI->load->database('db3', TRUE);			
			
				$borwseSQL1 = "SELECT * FROM `game_history` where play_group_id = '$handID'";
        $queryHH = $this->db3->query($borwseSQL1);
		$gameHandHis  =  $queryHH->row(); 
	
		
		//getting RIT table cards
		 $totalGamePlayers = $gameHandHis->TOTAL_PLAYERS; 
		$gamePlayGroupId  =$gameHandHis->PLAY_GROUP_ID;
		$totalRITCount = count($results);
		$actualRITCount = $totalGamePlayers * 2;
		
	
		if($totalGamePlayers < $totalRITCount){
		  //echo "test"; die;
		  $need = $totalGamePlayers + 1;
		  $n = $need-1;
		  $getRitQuery = "select h.PLAY_ID,h.USER_ID,h.PLAY_GROUP_ID,h.TOURNAMENT_ID,h.TOURNAMENT_TABLE_ID,h.INTERNAL_REFERENCE_NO,h.GAME_REFERENCE_NO,h.STAKE,h.WIN,h.REVENUE,h.RAKE,h.POT_AMOUNT,h.TABLE_CARD,h.PLAYER_CARD,h.WIN_TYPE,h.WIN_RESULT_TYPE,h.WIN_HANDS,h.USER_HAND,h.USER_ORDERS,h.PRE_FLOP,h.FLOP,h.TURN,h.RIVER,h.CREATED_DATE,u.USERNAME from hand_history as h,user as u where h.PLAY_GROUP_ID = '$gamePlayGroupId' order by h.HAND_HISTORY_ID desc LIMIT 1";
		  
		 $getRitQQ = $this->db3->query($getRitQuery);
		 $getRITTableCardInfo = $getRitQQ->row(); 
		  $ritCount = count($getRITTableCardInfo); 
		  if($ritCount > 0){
		 
			$ritTableCard = $getRITTableCardInfo->TABLE_CARD;
			if($ritTableCard != ''){
			   $explodeRitTableCards  = explode(",",$ritTableCard);
			   foreach($explodeRitTableCards as $ritTableCard){
					  echo '<p style="padding-left:0;'.$borderstyle.'"><img src="'.base_url().'static/images/minigame/poker/'.$ritTableCard.'.png"></p>';			
			   }
			}				
		  }
		}
...
<!--            <div style="float: left;width: 160px;"><h3>Real Revenue:</h3><span><?php //echo $real_revenue;?></span></div>
            <div style="float: left;width: 160px;"><h3 style="width: 105px;">Bonus Revenue:</h3><span style="width: 10px;"><?php //echo $bonus_revenue;?></span></div>-->
            </div>    
      ...
      <div class="container_searchgame">
    	<ul>
            ...
            <li style="min-width:195px !important;"><h3 class="header_searchgame">Hand ID</h3></li>
            <li style="min-width:30px"><h3 class="header_searchgame" style="min-width:35px">Bet</h3></li>
            <li style="min-width:30px"><h3 class="header_searchgame" style="min-width:35px">Status</h3></li>
            <li style="min-width:30px"><h3 class="header_searchgame" style="min-width:35px">Rake</h3></li>
             <li style="min-width:30px"><h3 class="header_searchgame" style="min-width:35px">Real Rake</h3></li>
              <li style="min-width:30px"><h3 class="header_searchgame" style="min-width:35px">Bonus Rake</h3></li>
            <li style="min-width: 162px;"><h3 class="header_searchgame">Card</h3></li>
        </ul>
    	<?php 
		if(isset($results)){ 
  			if(count($results)>0 && is_array($results)){
				...
				foreach($results as $resData){
					 ...
					 $TURN       = $resData->TURN; 
					 $RIVER      = $resData->RIVER; 
					 $realrevenue = $resData->REAL_REVENUE;
					 $bonusrevenue =  $resData->BONUS_REVENUE;
    ...
    <li style="min-height:45px;min-width:131px">
      <div class="searchgame_cardcnt">
        <p class="usrname"><?php echo $playerName; ?>
        <?php if($totalGamePlayers < $totalRITCount){ ?>
        <span style="color: red; position: relative; top: 20px; font-weight: bold; font-size: 11px;"><?php if($y > $totalGamePlayers) echo "(Round 2)"; else echo "(Round 1)"; ?></span>
        <?php } ?>
        </p>
      </div>
    </li>
    ...
    <li style="min-height:45px;min-width:88px">
      <div class="searchgame_cardcnt">
        <p class="usrname"><?php echo $stake; ?></p>
      </div>
    </li>
    <li style="min-height:45px;min-width:94px">
      <div class="searchgame_cardcnt">
        <p class="usrname">
    ...
    <li style="min-height:45px;min-width:22px">
      <div class="searchgame_cardcnt">
        <p class="usrname"><?php echo $revenue; ?></p>
      </div>
    </li>
    <li style="min-height:45px;min-width:104px">
      <div class="searchgame_cardcnt">
        <p class="usrname"><?php echo $realrevenue; ?></p>
      </div>
    </li>
    <li style="min-height:45px;min-width:99px">
      <div class="searchgame_cardcnt">
        <p class="usrname"><?php echo $bonusrevenue; ?></p>
      </div>
    </li>
    <li style="min-width:208px !;">
      <div class="searchgame_cardcnt">
        <?php
		if($playerCards != ''){
...
<?PHP
 if(isset($results)){ 
	  ...
	  foreach($results as $resData2B){
		...
		$playerName2B = $resData2B->USERNAME; 
