...
if(isset($results)){ 
	if(count($results)>0 && is_array($results)){
		...
		//call model to get Tournament Name,currency type.
			...
			if($results[$i]->TOURNAMENT_STATUS == 0){
			   if($results[$i]->IS_ACTIVE==1){
						$resvalue['ACTION']= '<a  href="'.base_url().'reports/tournamentreports/recurrenceInfo/'.$results[$i]->TOURNAMENT_RECURENCE_ID.'" onclick="return hs.htmlExpand(this, { objectType: \'iframe\' } )"><img src="'.base_url().'static/images/tourInfo.png" title="view"></a> &nbsp;&nbsp;&nbsp; ';
						
						$resvalue['ACTION']			.= '<a onclick="return confirm(\'Are you sure you want to delete this recurrence tournament?\');" href="'.base_url().'games/poker/tournament/deleteRecurrenceTournamentTable/'.$results[$i]->TOURNAMENT_RECURENCE_ID.'"><img style="width:20px;" src="'.base_url().'static/images/delete-recurre.png"/></a>';
			   }else{
			     $resvalue['ACTION']			=  '-';  
			   }
			}else{
    ...
    <div class="tableListWrap">
      <div class="data-list">
        ...
        <script type="text/javascript">
                jQuery("#list4").jqGrid({
                    datatype: "local",
					colNames:['Table ID','Stake','Game Type','Limit','Entry Fee','Comm.(%)','Starting Chips','Currency','Action'],
                    colModel:[
						...
						{name:'ENTRY_FEE',index:'ENTRY_FEE', align:"center", width:50,sorttype:"float"},
						{name:'COMMISSION',index:'COMMISSION', align:"center", width:40},
						{name:'STARTING_CHIPS',index:'STARTING_CHIPS', align:"center", width:50},
						{name:'CURRENCY',index:'CURRENCY', align:"center", width:30},
